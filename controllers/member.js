...
const getProgramsOfAMember = async (req, res) => {
  ...
  try {
    ...
    const members = await pool
      ...
      .query(
        "SELECT m.Id, m.StudentId, st.ClassCode, st.FullName, m.ProgramId, pr.Code, pr.Name, pr.CreatedDate, pr.UpdatedDate, pr.Description, pr.Image FROM Member AS m JOIN Program as pr ON m.ProgramId = pr.Id JOIN Student AS st on m.SchoolId = st.Id WHERE m.StudentId = @StudentId AND m.Status = 1 AND pr.Status = 1 AND st.Status = 1"
      );
    if (members.recordset.length === 0) {
      res.status(404).json({ error: "Member not found" });
    } else {
